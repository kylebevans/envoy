...
public:
  ...
  /**
   * Parse an internet host address (IPv4 or IPv6), create an Instance from it AND set the port.
   * Throws EnvoyException if unable to parse the address. This is needed when a shared pointer is
   * needed but only a raw instance is available.
   * @param Address::Ip& to be copied to the new instance.
   * @return pointer to the Instance.
   */
  static Address::InstanceConstSharedPtr copyInternetAddressAndSetPort(const Address::Ip& ip,
                                                                       const uint32_t port);
