...
private:
  ...
  struct PendingResolutionBase : public ActiveDnsQuery {
    ...
    PendingResolutionBase(DnsResolverImpl& parent, Event::Dispatcher& dispatcher, ares_channel channel,
                          const std::string& dns_name)
        : parent_(parent), dispatcher_(dispatcher), channel_(channel), dns_name_(dns_name) {}
   ...
   */
  ActiveDnsQuery* preparePendingResolution(std::unique_ptr<PendingResolutionBase> pending_resolution);
